...
1. The simplest way to run the project is using Docker containers which run the full-fledged VulnerableApplication with all the components. For running as a Docker application, follow these steps:
...
This script will build your local changes into a Docker image (`sasanlabs/owasp-vulnerableapp:unreleased`) and run `docker compose -f docker-compose.local.yml up` to start the full stack (including facade, jsp, and php services).
